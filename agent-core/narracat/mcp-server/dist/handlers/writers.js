...
import { validateCommitChapter, validateExtraction, validateConsolidate, validateReview, validateOutlinePayload, validateOutlineBookPayload, validateChapterOutlineBatch, validateForeshadowingItem, checkOutlineSemantics, checkOutlineBudget, checkStructureRhythm, checkDilemmaMilestones, checkForeshadowingPayoffTiming, checkChapterBatch, checkChapterProseHygiene, checkChapterWordCount, checkStateChanges, checkPayoffIntensityConsistency, checkOpeningRetention, checkOpeningArcPayoff, checkHookCadence, HOOK_CADENCE_THRESHOLDS, computeStructureBudget, validatePremise, checkPremiseSemantics, checkNarratorAddress, validateDialogueSamples, } from "./validators.js";
...
function findRealizedPlant(ctx, foreshadowingId, maxChapter) {
    const row = ctx.db
        .prepare(`SELECT chapter FROM foreshadowing_actions_log
       WHERE novel_id = ? AND foreshadowing_id = ? AND action = 'plant'
         AND status = 'realized' AND chapter <= ?
       ORDER BY chapter LIMIT 1`)
        .get(ctx.novelId, foreshadowingId, maxChapter);
    return row ? { chapter: row.chapter } : null;
}
function registryIds(ctx) {
    const rows = ctx.db
...
export function refreshCharacterCards(ctx, characters, asOfChapter) {
    const refreshed = [];
    const upsert = ctx.db.prepare(`INSERT INTO character_cards (novel_id, character_uid, character, as_of_chapter, card_json)
     VALUES (?, ?, ?, ?, ?)
...
// ============================================================
export async function novelCommitChapter(args, ctx) {
    ...
    const keyEvents = args["key_events"];
    const charactersAppeared = args["characters_appeared"];
    const emotionalTone = args["emotional_tone"];
    const continuationHook = args["continuation_hook"];
    const foreshadowingActions = args["foreshadowing_actions"] ?? [];
    const timelineNote = args["timeline_note"] ?? null;
    // 机械补全：读工作正文实数（staging 优先，无 staging 落回正式路径）
    const manuscript = await resolveWorkingManuscript(ctx.projectRoot, chapter);
    if (!manuscript) {
        return singleError("chapter", `manuscript/vol-VV/ch-${chapterFileSegment(chapter)}.md 存在`, "正文文件未找到", `第${chapter}章正文不存在：先确认 chapter-writer 已写入本章正文，再提交收尾`);
    }
    ...
    if (wordCount === 0) {
        return singleError("chapter", "非空正文", "0 字", `第${chapter}章正文文件为空，先补写正文再提交收尾`);
    }
    const { opening, ending } = extractManuscriptSnippets(raw);
    // 伏笔预检：id 必须已注册
    const knownIds = registryIds(ctx);
    const unknownErrors = foreshadowingActions
    ...
    if (unknownErrors.length > 0)
        return errorResponse(unknownErrors);
    ...
    const tx = ctx.db.transaction(() => {
        ...
        const insertAction = ctx.db.prepare(`INSERT OR IGNORE INTO foreshadowing_actions_log
    ...
    const receiptPath = join(ctx.projectRoot, ".narracat", "receipts", `ch-${chapterFileSegment(chapter)}.json`);
    const checklist = {
        chapter,
 ...
 */
async function commitResolvedFacts(facts, chapter, ctx, warnings) {
    // 预生成 embedding（事务外）
    const inserts = [];
    for (const f of facts) {
        if (f.change_type === "invalidate")
            continue;
    ...
    const tx = ctx.db.transaction(() => {
       ...
       WHERE id = ? AND novel_id = ?`);
        const insert = ctx.db.prepare(`INSERT INTO facts (id, novel_id, subject, subject_character_uid, subject_character_b_uid, predicate, object, sector, from_chapter, event_chapter, source)
        ...
        for (const f of facts) {
            if (f.change_type === "update" || f.change_type === "invalidate") {
                ...
                if (existing) {
                    ...
                    const replacedBy = f.change_type === "update" ? (insertByFact.get(f)?.id ?? null) : null;
                    invalidate.run(chapter, replacedBy, existing.id, ctx.novelId);
                    invalidated += 1;
                }
                else if (f.change_type === "invalidate") {
                    warnings.push(`invalidate 未命中：${f.subject} ${f.predicate} 没有可失效的旧值，已跳过`);
                    continue;
                }
                else {
            ...
            if (!op)
                continue;
            insert.run(op.id, ctx.novelId, f.subject, f.subject_character_uid, f.subject_character_b_uid, f.predicate, f.object, chapter, // from_chapter（ingestion：记录章）
            chapter);
...
// ============================================================
export async function novelSubmitExtraction(args, ctx) {
...
// ============================================================
export async function novelStageExtraction(args, ctx) {
    const runId = args["run_id"];
    if (typeof runId !== "number" || !Number.isInteger(runId) || runId < 1 || runId > 9) {
        return singleError("run_id", "integer 1-9", `${typeof runId}: ${JSON.stringify(runId)}`, "传入本轮抽取的 run_id（任务 envelope 里给定，照传，1-9）");
    }
    const validation = validateExtraction(args);
    if (!validation.valid)
        return errorResponse(validation.errors);
...
const CHANGE_TYPE_RANK = {
    new: 0,
    update: 1,
    invalidate: 2,
};
export async function novelCommitExtractionUnion(args, ctx) {
    const chapter = args["chapter"];
    if (typeof chapter !== "number" || !Number.isInteger(chapter) || chapter < 1) {
        return singleError("chapter", "integer ≥ 1", `${typeof chapter}: ${JSON.stringify(chapter)}`, "传入要落并集的章号");
    }
    // 机械合同门：staging 上的半成品不得进记忆（无 staging 免检，见 checkManuscriptContract）
    const contract = await checkManuscriptContract(ctx, chapter);
    if (contract && contract.errors.length > 0)
    ...
    if (freshnessErrors)
        return errorResponse(freshnessErrors);
    const rows = ctx.db
        .prepare(`SELECT facts_json FROM extraction_stage WHERE novel_id = ? AND chapter = ?`)
        .all(ctx.novelId, chapter);
    const warnings = [];
    const stagedRuns = rows.length;
    if (stagedRuns === 0) {
        // 三轮抽取均未暂存（疑似全部失败/跳过）——本章未沉淀任何事实，收尾不应据此判定完成
        warnings.push("该章无任何暂存事实：抽取暂存为空（疑似各轮均失败或被跳过），本章未沉淀事实");
    }
...
// ============================================================
export async function novelConsolidate(args, ctx) {
    ...
    if (scope === "arc") {
        const arc = ctx.db
            .prepare(`SELECT chapter_start, chapter_end FROM arc_meta WHERE novel_id = ? AND arc_id = ?`)
            .get(ctx.novelId, scopeId);
        if (!arc) {
            const known = ctx.db
                .prepare(`SELECT arc_id FROM arc_meta WHERE novel_id = ? ORDER BY chapter_start`)
                .all(ctx.novelId).map((r) => r.arc_id);
            return singleError("scope_id", "已注册的 arc_id", scopeId, `arc ${scopeId} 不存在。可用 arc_id：${known.join(", ") || "(空，先提交书级大纲)"}`);
        }
        ...
        if (!m) {
            return singleError("scope_id", 'scope=volume 时形如 "vol-01"', scopeId, '卷标识用 "vol-NN" 形态（如 "vol-01"）');
        }
        const volumeNo = parseInt(m[1], 10);
        const range = ctx.db
            .prepare(`SELECT MIN(chapter_start) AS s, MAX(chapter_end) AS e
         FROM arc_meta WHERE novel_id = ? AND volume_no = ?`)
            .get(ctx.novelId, volumeNo);
        if (range.s === null || range.e === null) {
            return singleError("scope_id", "已提交大纲的卷号", scopeId, `第 ${volumeNo} 卷没有任何 arc 记录：先用 novel_submit_outline 提交该卷大纲`);
        }
    ...
    const tx = ctx.db.transaction(() => {
        ...
        ctx.db
            .prepare(`INSERT INTO arc_summaries (novel_id, scope, scope_id, chapter_start, chapter_end, summary)
         VALUES (?, ?, ?, ?, ?, ?)
        ...
        // 区间内出场角色 → 机械折叠刷新状态卡
        const rows = ctx.db
            .prepare(`SELECT characters FROM chapter_summaries
         WHERE novel_id = ? AND chapter BETWEEN ? AND ?`)
            .all(ctx.novelId, chapterStart, chapterEnd);
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    for (const ref of parsed) {
                        const uid = ref && typeof ref === "object"
                            ? ref.character_uid
                            : undefined;
...
export async function novelSubmitReview(args, ctx) {
    ...
    const verdict = blockers.length > 0 ? "fail" : "pass";
    ...
    const reportPath = join(ctx.projectRoot, "reviews", `ch-${chapterFileSegment(chapter)}-review.json`);
    await mkdir(dirname(reportPath), { recursive: true });
    await writeFile(reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf-8");
    // 路由信号入库（novel_get_review 读取）
    ctx.db
        .prepare(`INSERT INTO chapter_reviews (novel_id, chapter, verdict, issues_json, reviewed_manuscript_sha256)
       VALUES (?, ?, ?, ?, ?)
    ...
    return {
        ...
        message: `第${chapter}章审校结果 ${verdict.toUpperCase()}（blocker ${blockers.length} / note ${notes.length}）` +
...
// ============================================================
export async function backfillReviewArtifacts(ctx) {
    const rows = ctx.db
        .prepare(`SELECT chapter, verdict, issues_json, reviewed_manuscript_sha256
       FROM chapter_reviews WHERE novel_id = ?`)
        .all(ctx.novelId);
    for (const row of rows) {
        const reportPath = join(ctx.projectRoot, "reviews", `ch-${chapterFileSegment(row.chapter)}-review.json`);
...
const PREMISE_PROSE_CARDS = new Set([
    "core_hook",
    "antagonistic_force",
    "central_dramatic_question",
]);
/** 非 canon（含未标注视为 canon）才标注，供 agent 识别暂定/留白；canon 不标 */
function premiseCertaintyTag(certainty) {
    if (!certainty || certainty === "canon")
        return "";
    return ` [${certainty}]`;
}
/**
 ...
 */
export function renderPremiseMarkdown(payload) {
    const byCard = new Map();
    for (const card of payload.cards)
        byCard.set(card.card, card.fields);
    const lines = ["# 立项卡", ""];
            ...
            for (const f of fields) {
                if (f.key === "reference_example") {
                    ...
                    if (f.note && f.note.trim())
                        lines.push(`  mechanism_note: ${f.note.trim()}`);
                }
                else {
    ...
    if (tentative.length === 0 && open.length === 0) {
        ...
        if (tentative.length > 0)
            lines.push(`- 暂定: ${tentative.join("、")}`);
        if (open.length > 0)
            lines.push(`- 留白: ${open.join("、")}`);
        lines.push("");
    }
    return `${lines.join("\n").trimEnd()}\n`;
}
...
function collectPremiseEngineOverlap(payload) {
    const byCard = new Map();
    for (const card of payload.cards)
        byCard.set(card.card, card.fields);
    const overlap = [];
...
async function syncPremiseEngineFacts(overlap, ctx) {
    ...
    const tx = ctx.db.transaction(() => {
        for (const op of ops) {
            ...
            ctx.db
                .prepare(`INSERT INTO facts (id, novel_id, subject, predicate, object, sector, from_chapter, source)
 ...
 */
async function persistOutlineStructure(ctx, payload) {
 ...
 */
async function syncPremiseToOutlineRender(overlap, ctx) {
    const structurePath = join(ctx.projectRoot, "outline", "outline-structure.json");
    if (!existsSync(structurePath))
        return false;
    let payload;
    try {
        payload = JSON.parse(await readFile(structurePath, "utf-8"));
    }
    catch {
 ...
 */
function mergePremiseCards(submitted, ctx) {
    const byCard = new Map();
    const row = ctx.db
    ...
    if (row) {
        try {
            const existing = JSON.parse(row.cards_json);
            if (Array.isArray(existing))
                for (const card of existing)
                    byCard.set(card.card, card);
        }
        catch {
            // 坏存量 cards_json 忽略，按全量提交处理
        }
    }
    for (const card of submitted.cards)
        byCard.set(card.card, card);
    return { cards: [...byCard.values()] };
}
export async function novelSubmitPremise(args, ctx) {
    ...
    const addressErrors = checkNarratorAddress(rawPayload, {
        requirePresence: args["merge_cards"] !== true,
    });
    if (addressErrors.length > 0)
        return errorResponse(addressErrors);
    // 增量合并：定点修订只提交目标卡时，先并入现有立项卡再校验/写回（避免漏卡丢失其余卡）
    const payload = args["merge_cards"] === true
        ? mergePremiseCards(rawPayload, ctx)
        : rawPayload;
    const semanticErrors = checkPremiseSemantics(payload);
    if (semanticErrors.length > 0)
        return errorResponse(semanticErrors);
    // 结构化真相入库：upsert cards_json（单行/小说，与 chapter_reviews 同模式）
    ctx.db
        .prepare(`INSERT INTO premise_cards (novel_id, cards_json)
       VALUES (?, ?)
    ...
    return {
        ...
        message: `立项卡已入库：${payload.cards.length} 张卡（premise.md 已机械渲染为只读视图）${syncedFacts.length > 0 ? `；已同步 ${syncedFacts.length} 项全书 facts` : ""}${syncedOutline ? "、回灌大纲渲染" : ""}`,
    };
}
// ============================================================
...
// ============================================================
export async function backfillPremiseArtifacts(ctx) {
    const dataPath = join(ctx.projectRoot, "bible", "premise-cards.json");
...
export async function backfillOutlineArtifacts(ctx) {
    const structurePath = join(ctx.projectRoot, "outline", "outline-structure.json");
    if (existsSync(structurePath))
        return;
    // arc_meta 是书级大纲是否存在的判据：无 arc 即未提交大纲，跳过
    const arcRows = ctx.db
        .prepare(`SELECT arc_id, volume_no, title, chapter_start, chapter_end,
    ...
    for (const predicate of ENGINE_PREDICATES) {
        const row = ctx.db
            .prepare(`SELECT object FROM facts
         WHERE novel_id = ? AND subject = '全书' AND predicate = ?
         ORDER BY from_chapter DESC, created_at DESC LIMIT 1`)
            .get(ctx.novelId, predicate);
        engine[predicate] = row?.object ?? "";
    }
    // 故事线
    const storylineRows = ctx.db
        .prepare(`SELECT id, name, type, priority, entry_chapter, planned_payoff_chapter, status, is_through_line
    ...
    const registryRows = ctx.db
        .prepare(`SELECT id, type, description, planted_chapter, target_reveal, theme_link
    ...
    for (const arc of arcRows) {
        ...
        let beats = [];
        try {
            const parsed = JSON.parse(arc.payoff_beats);
            if (Array.isArray(parsed))
                beats = parsed.filter((b) => typeof b === "string");
        }
        catch {
            // 坏 JSON 按空爽点列表处理
        }
 ...
 */
async function readNarratorVoiceFromBible(projectRoot) {
    let content;
    try {
    ...
    const start = lines.findIndex((line) => /^#{2,4}\s*.*(叙述声音|叙述者腔调)/.test(line.trim()));
    if (start === -1)
        return null;
    ...
    for (const line of lines.slice(start + 1, end)) {
        ...
        if (key === "mechanism_note") {
            ...
            if (current)
                current.mechanism_note = value;
            continue;
        }
        if (NARRATOR_KEYS.includes(key) && !values.has(key)) {
            values.set(key, value);
        }
    }
    if (values.size === 0 && examples.length === 0)
        return null;
    return { values, examples };
}
function renderNarratorVoiceSection(data) {
    const lines = ["## 叙述者腔调", ""];
    if (!data) {
    ...
    for (const key of NARRATOR_KEYS) {
        ...
        lines.push(NARRATOR_BOLD_KEYS.has(key) ? `- **${key}**: ${value}` : `- ${key}: ${value}`);
    }
...
export function renderMasterOutlineMarkdown(payload, narratorVoice) {
    const masterLines = ["# 全书大纲", ""];
    for (const [key, label] of ENGINE_FIELDS) {
        masterLines.push(`## ${label}`, "", payload[key], "");
    }
    ...
    for (const sl of [...payload.storylines].sort((a, b) => a.priority - b.priority)) {
        ...
        masterLines.push(`- ${sl.id}「${sl.name}」（${label}，优先级 ${sl.priority}）：第 ${sl.entry_chapter} 章入场${payoff}${throughLine}`);
    }
    masterLines.push("");
    masterLines.push("## 伏笔注册表", "");
    for (const fs of payload.foreshadowing_registry) {
        const theme = fs.theme_link ? `，主题联系：${fs.theme_link}` : "";
        masterLines.push(`- ${fs.id}（${fs.type}）：${fs.description} —— 埋设第 ${fs.planted_chapter} 章，计划兑现 ${fs.target_reveal}${theme}`);
    }
    masterLines.push("");
    ...
    if (payload.volumes.length === 0) {
        ...
        for (const vol of sortedVolumes) {
            ...
            for (const arc of arcs) {
                masterLines.push(`- ${arc.arc_id} ${arc.title}（第 ${arc.chapter_start}-${arc.chapter_end} 章）：${arc.core_question}`);
            }
            masterLines.push("");
        }
    }
 ...
 */
function checkWrittenStructureIdentity(payload, scope, ctx) {
    const errors = [];
    // 剧情弧门 + 卷门：book 段 volumes 恒为占位空数组（不删任何 arc / 卷），天然豁免。
    if (scope !== "book") {
        const submittedArcIds = new Set(payload.volumes.flatMap((vol) => vol.arc_list.map((arc) => arc.arc_id)));
        const existingArcs = ctx.db
            .prepare(`SELECT arc_id, chapter_start, chapter_end FROM arc_meta WHERE novel_id = ?`)
            .all(ctx.novelId);
        const hasWrittenChapterInRange = ctx.db.prepare(`SELECT 1 FROM chapter_summaries WHERE novel_id = ? AND chapter BETWEEN ? AND ? LIMIT 1`);
        // consolidate 落 arc 摘要前已验 arc_id 注册在案，scope_id === arc_id 精确匹配可靠。
        const hasArcSummary = ctx.db.prepare(`SELECT 1 FROM arc_summaries WHERE novel_id = ? AND scope = 'arc' AND scope_id = ? LIMIT 1`);
        for (const arc of existingArcs) {
            if (submittedArcIds.has(arc.arc_id))
                continue;
            const written = hasWrittenChapterInRange.get(ctx.novelId, arc.chapter_start, arc.chapter_end) ??
        ...
        const removedVolumeNos = new Set(ctx.db
    ...
    const existingStorylines = ctx.db
        ...
        .all(ctx.novelId);
    const hasWrittenFocusRef = ctx.db.prepare(`SELECT 1 FROM chapter_storyline_focus f
    ...
    for (const sl of existingStorylines) {
        if (submittedStorylineIds.has(sl.id))
            continue;
...
export async function novelSubmitOutline(args, ctx) {
    ...
    if (scope === "volumes") {
        ...
        let existing;
        try {
            existing = JSON.parse(await readFile(structurePath, "utf-8"));
        }
        catch {
            return singleError("outline", "outline/outline-structure.json 可解析", "JSON 解析失败", `大纲结构文件已损坏：先用 scope="book" 重新提交书级骨架修复该文件，确认后再展开卷级`);
        }
    ...
    if (scope === "book") {
        ...
        if (existsSync(existingStructurePath)) {
            let existingVolumes;
            try {
                existingVolumes = JSON.parse(await readFile(existingStructurePath, "utf-8"))["volumes"];
            }
            catch {
                existingVolumes = undefined; // 坏 JSON 不阻断：book 段重写恰可修复该文件
            }
            if (Array.isArray(existingVolumes) && existingVolumes.length > 0) {
                return singleError("scope", "book 段仅限卷级未展开的书", `outline-structure.json 已含 ${existingVolumes.length} 卷`, "卷级已展开：书级定点修改走 novel_update_outline_book_field，结构调整走全量提交（scope 缺省）");
            }
        }
        const submittedVolumes = rawPayload["volumes"];
        if (Array.isArray(submittedVolumes) && submittedVolumes.length > 0) {
            return singleError("payload.volumes", "书级段不含 volumes", `${submittedVolumes.length} 卷`, `书级段只提交引擎字段 + storylines + 伏笔注册表；卷级在书级确认后用 scope="volumes" 展开`);
        }
        payload = { ...rawPayload, volumes: [] };
    }
    const warnings = [];
    const errors = [];
    ...
    if (ctx.estimatedTotalChapters && ctx.wordsPerChapter) {
        ...
        warnings.push("config.yaml 缺 estimated_total_chapters / words_per_chapter，结构预算核验已跳过");
    ...
    const tx = ctx.db.transaction(() => {
        ...
        // storylines upsert
        const upsertStoryline = ctx.db.prepare(`INSERT INTO storylines
       ...
       ON CONFLICT(novel_id, id) DO UPDATE SET
        ...
        for (const sl of payload.storylines) {
            upsertStoryline.run(ctx.novelId, sl.id, sl.name, sl.type, sl.priority, sl.entry_chapter, sl.planned_payoff_chapter ?? null, sl.status ?? "active", sl.is_through_line ? 1 : 0);
        }
        // 伏笔注册（保留既有 planted_chapter——后章重复 plant 由生命周期链矫正）
        const upsertForeshadowing = ctx.db.prepare(`INSERT INTO foreshadowing_registry
       ...
       ON CONFLICT(novel_id, id) DO UPDATE SET
        ...
        for (const fs of payload.foreshadowing_registry) {
            upsertForeshadowing.run(ctx.novelId, fs.id, fs.type, fs.description, fs.planted_chapter, fs.target_reveal, fs.theme_link ?? null);
        }
        // arc_meta upsert
        const upsertArc = ctx.db.prepare(`INSERT INTO arc_meta
       ...
       ON CONFLICT(novel_id, arc_id) DO UPDATE SET
        ...
        for (const vol of payload.volumes) {
            for (const arc of vol.arc_list) {
                upsertArc.run(ctx.novelId, arc.arc_id, vol.volume_no, arc.title, arc.chapter_start, arc.chapter_end, arc.core_question, arc.irreversible_change, arc.next_arc_seed, arc.antagonist_agent ?? null, JSON.stringify(arc.payoff_beats));
                arcCount += 1;
            }
        }
    ...
    const allArcs = ctx.db
        .prepare(`SELECT arc_id, volume_no, chapter_start, chapter_end
    ...
    if (fullCoverage) {
        const written = await writeStructureToState(ctx.projectRoot, maxVolume, maxChapter, chapterToVolume);
        if (written.ok) {
    ...
    for (const vol of sortedVolumes) {
        ...
        const volPath = join(ctx.projectRoot, "outline", volumeDirSegment(vol.volume_no), "vol-outline.md");
    ...
    if (scope !== "book") {
        ...
        for (const entry of outlineEntries) {
            ...
            if (submittedVolumeNos.has(volNo))
                continue;
            ...
            if (remaining.length === 0) {
                ...
                warnings.push(`第 ${volNo} 卷已不在大纲结构中，其卷级大纲文件已清理；该卷章级细纲文件仍保留，需人工确认处置`);
            }
        }
    }
    const message = scope === "book"
...
export async function novelSubmitChapterOutline(args, ctx) {
    ...
    let opening = { errors: [], warnings: [] };
    const openingArc = arcRows.find((a) => a.chapter_start === 1);
    if (openingArc &&
        chapters.some((c) => c.chapter >= openingArc.chapter_start && c.chapter <= openingArc.chapter_end)) {
        ...
        for (let c = openingArc.chapter_start; c <= openingArc.chapter_end; c += 1) {
            ...
            const jsonPath = join(ctx.projectRoot, "outline", volumeDirSegment(openingArc.volume_no), `ch-${chapterFileSegment(c)}.json`);
            if (existsSync(jsonPath)) {
                try {
                    mergedOpening.push(JSON.parse(await readFile(jsonPath, "utf-8")));
                }
                catch {
    ...
    for (let c = hookLo; c <= hookHi; c += 1) {
        ...
        if (!arcOfChapter)
            continue; // 章号不在任何 arc → 视同未规划
        const storedPath = join(ctx.projectRoot, "outline", volumeDirSegment(arcOfChapter.volumeNo), `ch-${chapterFileSegment(c)}.json`);
        if (!existsSync(storedPath))
            continue;
        try {
            const stored = JSON.parse(await readFile(storedPath, "utf-8"));
    ...
    if (blockingErrors.length > 0)
        return errorResponse(blockingErrors);
    const warnings = [
        ...check.warnings,
        ...opening.warnings,
    ...
    const tx = ctx.db.transaction(() => {
        const clearFocus = ctx.db.prepare(`DELETE FROM chapter_storyline_focus WHERE novel_id = ? AND chapter = ?`);
        const insertFocus = ctx.db.prepare(`INSERT OR IGNORE INTO chapter_storyline_focus (novel_id, chapter, storyline_id)
       VALUES (?, ?, ?)`);
        const insertPlanned = ctx.db.prepare(`INSERT OR IGNORE INTO foreshadowing_actions_log
         (novel_id, chapter, foreshadowing_id, action, status)
       VALUES (?, ?, ?, ?, 'planned')`);
        ...
        for (const ch of chapters) {
            ...
            mirrorChapterPlannedState(ctx.db, ctx.novelId, ch.chapter, (ch.state_changes ?? []).map((sc) => ({
    ...
    for (const ch of [...chapters].sort((a, b) => a.chapter - b.chapter)) {
        ...
        if (!arc)
            continue; // checkChapterBatch 已保证覆盖；防御性跳过
...
// ============================================================
export async function novelRegisterForeshadowing(args, ctx) {
    ...
    const plantedChapter = args["planted_chapter"];
    const targetReveal = args["target_reveal"];
    const themeLink = args["theme_link"] ?? null;
    const existing = ctx.db
        .prepare(`SELECT planted_chapter FROM foreshadowing_registry WHERE novel_id = ? AND id = ?`)
        .get(ctx.novelId, id);
    ctx.db
        .prepare(`INSERT INTO foreshadowing_registry
         (novel_id, id, type, description, planted_chapter, target_reveal, theme_link)
       VALUES (?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT(novel_id, id) DO UPDATE SET
         type = excluded.type,
    ...
    return {
        ok: true,
        id,
        lifecycle_guard: existingPlanted !== undefined &&
...
export async function novelUpdateOutlineBookField(args, ctx) {
    const target = args["target"];
    const id = typeof args["id"] === "string" ? args["id"] : undefined;
    const newValue = typeof args["new_value"] === "string" ? args["new_value"].trim() : "";
    const expected = typeof args["expected_old_value"] === "string"
        ? args["expected_old_value"].trim()
        : "";
    if (!OUTLINE_BOOK_FIELD_TARGETS.includes(target)) {
        return errorResponse([
            {
                field: "target",
                expected: OUTLINE_BOOK_FIELD_TARGETS.join(" | "),
                actual: `${typeof target}: ${JSON.stringify(target)}`,
                hint: "立项卡映射字段（中心戏剧问题 / 主角核心欲望 / 主角核心缺失 / 对抗力量）不在本工具范围，" +
    ...
    const payload = JSON.parse(await readFile(structurePath, "utf-8"));
    // 定位当前值 + CAS（三分支互斥定位，均取不到即视为「条目不存在」）
    let current;
    if (validatedTarget === "stakes_progression")
        current = payload.stakes_progression;
    ...
    if (validatedTarget === "stakes_progression") {
        ...
        const tx = ctx.db.transaction(() => {
            const stale = ctx.db
                .prepare(`SELECT id FROM facts WHERE novel_id = ? AND subject = '全书' AND predicate = ?`)
                .all(ctx.novelId, "stakes_progression");
            const staleIds = stale.map((r) => r.id);
            ftsDeleteBySourceIds(ctx.db, "facts", staleIds);
            vecDeleteBySourceIds(ctx.db, "facts", staleIds);
        ...
        nextPayload = {
            ...payload,
            foreshadowing_registry: payload.foreshadowing_registry.map((f) => f.id === id ? { ...f, description: newValue } : f),
        };
        ctx.db
...
export async function novelRollbackChapter(args, ctx) {
    ...
    const tx = ctx.db.transaction(() => {
        ...
        const actions = ctx.db
            .prepare(`DELETE FROM foreshadowing_actions_log
        ...
        const staleScopes = ctx.db
            .prepare(`SELECT scope, scope_id FROM arc_summaries WHERE novel_id = ? AND chapter_end >= ?`)
            .all(ctx.novelId, chapter);
        ...
        const staleCards = ctx.db
            .prepare(`SELECT character_uid, character FROM character_cards WHERE novel_id = ? AND as_of_chapter >= ?`)
            .all(ctx.novelId, chapter);
...
export async function novelSubmitDialogueSamples(args, ctx) {
    ...
    for (const name of unresolved) {
        warnings.push(`角色「${name}」未解析到 character_uid，台词样本已跳过：确认该名与 bible/characters/ 档案文件名或「别名:」声明一致`);
    }
    let stored = 0;
    const tx = ctx.db.transaction(() => {
        const stmt = ctx.db.prepare(`INSERT OR IGNORE INTO character_dialogue_samples
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        for (const { sample, uid, name } of resolved) {
            const info = stmt.run(randomUUID(), ctx.novelId, chapter, name, uid, sample.dialogue_text, sample.context ?? null, sample.emotion ?? null, sample.dialogue_type, sample.position_in_chapter ?? null);
            stored += info.changes;
        }
    });
    tx();
    return {
        ...
        warnings,
        message: `第${chapter}章台词语料已入库（新增 ${stored} 条${unresolved.size > 0 ? `，跳过 ${unresolved.size} 个未建档角色` : ""}）`,
    };
}
