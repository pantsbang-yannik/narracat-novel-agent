...
async function resolveExtractionFacts(
  ...
  | { errors: ToolErrorItem[]; facts?: undefined; warnings?: undefined }
  | { facts: ResolvedFact[]; warnings: string[]; errors?: undefined }
> {
  const aliasMap = await loadAliasMap(ctx.projectRoot);
  const warnings: string[] = [];

  // relationship 两端必是已建档角色：uid 必达，否则返回可修正错误
  const unresolvedRels = relationshipUpdates.filter(
    (rel) => !resolveCharacterUid(rel.a, aliasMap) || !resolveCharacterUid(rel.b, aliasMap),
  );
...
export async function novelSubmitExtraction(
  ...
  if (resolved.errors) return errorResponse(resolved.errors);
