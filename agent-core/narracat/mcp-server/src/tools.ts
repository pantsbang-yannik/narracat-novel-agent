...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "书级 + 卷级大纲提交（outline-architect）：引擎 5 字段 + storylines + 伏笔注册表 + 卷与 arc。入口 ajv + 结构预算核验，通过后入库并机械渲染 master-outline.md 与 vol-outline.md。支持两段制：scope=book 只提交书级骨架（卷结构渲染为待展开）；scope=volumes 在书级确认后只提交卷级（书级以库内为准，不覆盖）",
    inputSchema: {
      type: "object",
      properties: {
        ...
        scope: {
          ...
          description:
            "提交形态（缺省 full）：full=书级+卷级一次提交；book=只提交书级骨架（payload 不含 volumes）；volumes=书级确认后只提交卷级（payload 只需 { volumes }，书级以库内为准）",
        },
        payload: {
          type: "object",
          description:
